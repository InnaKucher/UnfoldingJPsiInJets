...
void create(bool doPrompt = true, bool doMid = true, bool doTrain = false, Int_t stepNumber = 1){
  ...
  if(doPrompt && doMid) {
    if(!doTrain) inputName = Form("optUnf/unfInput/step%i/unfolding_4D_prompt_midRapidity_Test.root",stepNumber);
    else inputName = Form("optUnf/unfInput/step%i/unfolding_4D_prompt_midRapidity_Train.root",stepNumber);
  }
  if(doPrompt && !doMid) {
    if(!doTrain) inputName = Form("optUnf/unfInput/step%i/unfolding_4D_prompt_fwdRapidity_Test.root",stepNumber);
    else inputName = Form("optUnf/unfInput/step%i/unfolding_4D_prompt_fwdRapidity_Train.root",stepNumber);
  }

  if(!doPrompt && doMid) {
    if(!doTrain) inputName = Form("optUnf/unfInput/step%i/unfolding_4D_nonprompt_midRapidity_Test.root",stepNumber);
    else inputName = Form("optUnf/unfInput/step%i/unfolding_4D_nonprompt_midRapidity_Train.root",stepNumber);
  }
  if(!doPrompt && !doMid) {
    if(!doTrain) inputName = Form("optUnf/unfInput/step%i/unfolding_4D_nonprompt_fwdRapidity_Test.root",stepNumber);
    else inputName = Form("optUnf/unfInput/step%i/unfolding_4D_nonprompt_fwdRapidity_Train.root",stepNumber);
  }

  outputName = inputName;
  outputName.replace(22,9,partOfOutput);
  ...
  if(doTrain) thnSparseName = "hs_norm_z_genJetPt;1";
...
void createRooUnfoldResponseMC(Int_t step = 1){
