...
void prepare(bool doPrompt = false, bool doTrain=true, bool doMid = true, Int_t stepNumber = 1){
  ...
  if(doPrompt && doTrain){
    ...
    if(doMid) outputfile = Form("../unfInput/step%i/unfolding_4D_prompt_midRapidity_Train.root",stepNumber);
    if(!doMid) outputfile = Form("../unfInput/step%i/unfolding_4D_prompt_fwdRapidity_Train.root",stepNumber);
  ...
  if(doPrompt && !doTrain){
    ...
    if(doMid) outputfile = Form("../unfInput/step%i/unfolding_4D_prompt_midRapidity_Test.root",stepNumber);
    if(!doMid) outputfile = Form("../unfInput/step%i/unfolding_4D_prompt_fwdRapidity_Test.root",stepNumber);
  ...
  if(!doPrompt && doTrain){
    ...
    if(doMid) outputfile = Form("../unfInput/step%i/unfolding_4D_nonprompt_midRapidity_Train.root",stepNumber);
    if(!doMid) outputfile = Form("../unfInput/step%i/unfolding_4D_nonprompt_fwdRapidity_Train.root",stepNumber);
  ...
  if(!doPrompt && !doTrain){
    ...
    if(doMid) outputfile = Form("../unfInput/step%i/unfolding_4D_nonprompt_midRapidity_Test.root",stepNumber);
    if(!doMid) outputfile = Form("../unfInput/step%i/unfolding_4D_nonprompt_fwdRapidity_Test.root",stepNumber);
  ...
  for (Long64_t count = 0; count < fSparse->GetNbins(); ++count) {
    ...
    if(x[1] > 0.89 && x[1] < 0.91){
      ...
      cout << " >>> the highest z bin, the bin value = " << x[fDim] << endl;
...
void prepareInputsMC(Int_t step = 1){
